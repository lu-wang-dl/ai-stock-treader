...
class StockAnalysisAgents:
    ...
    def __init__(self, model="databricks-gemini-3-pro"):
        self.model = model
        self.databricks_client = DatabricksClient(model=model)
    ...
    def technical_analyst_agent(self, stock_info: Dict, stock_data: Any, indicators: Dict) -> Dict[str, Any]:
        ...
        analysis = self.databricks_client.technical_analysis(stock_info, stock_data, indicators)
    ...
    def fundamental_analyst_agent(self, stock_info: Dict, financial_data: Dict = None, quarterly_data: Dict = None) -> Dict[str, Any]:
        ...
        analysis = self.databricks_client.fundamental_analysis(stock_info, financial_data, quarterly_data)
    ...
    def fund_flow_analyst_agent(self, stock_info: Dict, indicators: Dict, fund_flow_data: Dict = None) -> Dict[str, Any]:
        ...
        analysis = self.databricks_client.fund_flow_analysis(stock_info, indicators, fund_flow_data)
...
"""
        ...
        analysis = self.databricks_client.call_api(messages, max_tokens=6000)
...
"""
        ...
        analysis = self.databricks_client.call_api(messages, max_tokens=4000)
...
"""
    ...
    def stock_picker_agent(self, num_stocks: int = 5,
                          sectors: Optional[List[str]] = None,
                          custom_symbols: Optional[List[str]] = None,
                          min_buy_signals: int = 2) -> Dict[str, Any]:
        """Stock picker agent - selects the best stocks to buy using AI.

        Args:
            num_stocks: Number of stocks to select.
            sectors: Sectors to source candidates from (None = all sectors).
            custom_symbols: Explicit list of symbols to consider.
            min_buy_signals: Minimum technical buy signals for pre-filtering.

        Returns:
            Dict with picks, reasoning, and metadata.
        """
        from stock_picker_agent import StockPickerAgent
        picker = StockPickerAgent(model=self.model)
        return picker.pick_stocks(
    ...
    def run_multi_agent_analysis(self, stock_info: Dict, stock_data: Any, indicators: Dict, snapshot: Any,
    ...
    def conduct_team_discussion(self, agents_results: Dict[str, Any], stock_info: Dict, snapshot) -> str:
        """进行团队讨论"""
        print("🤝 分析团队正在进行综合讨论...")
        time.sleep(2)
...
"""
        ...
        discussion_result = self.databricks_client.call_api(messages, max_tokens=6000)
        
        print("✅ 团队讨论完成")
        return discussion_result
    
    def make_final_decision(self, discussion_result: str, stock_info: Dict, indicators: Dict, snapshot) -> Dict[str, Any]:
        """制定最终投资决策"""
        print("📋 正在制定最终投资决策...")
        time.sleep(1)
        
        decision = self.databricks_client.final_decision(discussion_result, stock_info, indicators, snapshot)
