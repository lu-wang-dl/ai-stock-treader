...
class QuarterlyReportDataFetcher:
        ...
        if self._is_chinese_stock(symbol):
            ...
            try:
                if self._is_us_stock(symbol):
                    print(f"📊 正在获取美股 {symbol} 的季报数据...")
                
                income_data = self._get_income_statement_us(symbol)
                if income_data:
                    data["income_statement"] = income_data
                    print(f"   ✓ 成功获取 {len(income_data.get('data', []))} 期利润表数据")
                
                # 获取资产负债表
                balance_data = self._get_balance_sheet_us(symbol)
                if balance_data:
                    data["balance_sheet"] = balance_data
                    print(f"   ✓ 成功获取 {len(balance_data.get('data', []))} 期资产负债表数据")
                
                # 获取现金流量表
                cash_flow_data = self._get_cash_flow_us(symbol)
                if cash_flow_data:
                    data["cash_flow"] = cash_flow_data
                    print(f"   ✓ 成功获取 {len(cash_flow_data.get('data', []))} 期现金流量表数据")
                
                indicators_data = self._get_financial_statement_us(symbol)
                if indicators_data:
                    data["financial_indicators"] = indicators_data
                    print(f"   ✓ 成功获取 {len(indicators_data.get('data', []))} 期财务指标数据")
    ...
    def _is_us_stock(self, symbol):
        """
        判断是否为美股股票代码
        常见美股代码为: 
            - 1~5个大写字母 (AAPL, TSLA, MSFT, NVDA)
            - 一些ETF有点号（如BRK.B），此处简要判定
        """
        ...
        # 支持点号后缀，如"BRK.B"
        if '.' in symbol:
            main, ext = symbol.split('.', 1)
            if main.isalpha() and (1 <= len(main) <= 4) and ext.isalpha() and (1 <= len(ext) <= 2):
                return True
        return False
    ...
    def _get_income_statement_us(self, symbol):
        """获取美股利润表数据"""
        try:
            # stock_financial_us_report_em - 新浪财经季度利润表
            # 若美股代码包含点（如 BRK.A），需替换为下划线（BRK_A）
            api_symbol = symbol.replace('.', '_') if '.' in symbol else symbol
            df = ak.stock_financial_us_report_em(stock=api_symbol, symbol="综合损益表", indicator="单季报")
    ...
    def _get_balance_sheet_us(self, symbol):
        """获取资产负债表数据"""
        try:
            # stock_financial_us_report_em - 新浪财经季度利润表
            # 若美股代码包含点（如 BRK.A），需替换为下划线（BRK_A）
            api_symbol = symbol.replace('.', '_') if '.' in symbol else symbol
            df = ak.stock_financial_us_report_em(stock=api_symbol, symbol="资产负债表", indicator="单季报")
    ...
    def _get_cash_flow_us(self, symbol):
        """获取现金流量表数据"""
        try:
            # stock_financial_us_report_em - 新浪财经季度利润表
            # 若美股代码包含点（如 BRK.A），需替换为下划线（BRK_A）
            api_symbol = symbol.replace('.', '_') if '.' in symbol else symbol
            df = ak.stock_financial_us_report_em(stock=api_symbol, symbol="现金流量表", indicator="单季报")
    ...
    def _get_financial_statement_us(self, symbol):
        """获取美股财务指标数据"""
        try:
            # stock_financial_us_analysis_indicator_em - 美股财务指标
            # 若美股代码包含点（如 BRK.A），需替换为下划线（BRK_A）
            api_symbol = symbol.replace('.', '_') if '.' in symbol else symbol
            df = ak.stock_financial_us_analysis_indicator_em(symbol=api_symbol, indicator="单季报")
...
""")
            ...
            key_fields = ['报告期', '营业总收入', '营业收入', '营业总成本', '营业利润', 
                         ...
                         '基本每股收益', '稀释每股收益', 'REPORT_DATE', 'AMOUNT']
...
""")
            ...
            key_fields = ['报告期', '资产总计', '流动资产合计', '非流动资产合计',
                         '负债合计', '流动负债合计', '非流动负债合计',
                         '所有者权益合计', '归属于母公司股东权益合计', 'REPORT_DATE', 'AMOUNT']
...
""")
            ...
            key_fields = ['报告期', '经营活动产生的现金流量净额', 
                         ...
                         '现金及现金等价物净增加额', '期末现金及现金等价物余额', 'REPORT_DATE', 'AMOUNT']
...
""")
            ...
            key_fields = ['报告期', '净资产收益率', '总资产净利率', '销售净利率',
                         ...
                         '每股收益', '每股净资产', '每股经营现金流'] + [
                         'REPORT_DATE', 'OPERATE_INCOME', 'OPERATE_INCOME_YOY', 
                         'GROSS_PROFIT', 'GROSS_PROFIT_YOY', 'PARENT_HOLDER_NETPROFIT', 'PARENT_HOLDER_NETPROFIT_YOY',
                         'BASIC_EPS', 'GROSS_PROFIT_RATIO', 'NET_PROFIT_RATIO', 'TOTAL_ASSETS_TR', 'GROSS_PROFIT_RATIO_YOY',
                         'NET_PROFIT_RATIO_YOY', 'ROE_AVG_YOY', 'ROA_YOY', 'DEBT_ASSET_RATIO_YOY', 'CURRENT_RATIO_YOY', 'SPEED_RATIO_YOY']
